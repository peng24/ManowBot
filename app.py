...
def extract_data(text: str) -> dict | None:
    ...
    system_prompt = """\
คุณเป็น AI สำหรับดึงข้อมูลจากข้อความไลฟ์สดขายเสื้อผ้า
หน้าที่ของคุณคือดึง "รายการที่" (item) และ "ราคา" (price) จากข้อความที่ได้รับ

กฎ:
- item คือ หมายเลขรายการสินค้า (ตัวเลข)
- price คือ ราคาสินค้า (ตัวเลข)
- รองรับภาษาอีสาน เช่น:
  - "ซาว" = 20, "สิบ" = 10, "ร้อย" = 100, "พัน" = 1000, "บาก" = บาท
- ตัวอย่าง:
  - "รายการที่ 5 ราคา ซาว" → {"item": 5, "price": 20}
  - "ตัวที่ 3 สามร้อย" → {"item": 3, "price": 300}
  - "เบอร์ 12 ราคา 150" → {"item": 12, "price": 150}
- ตอบกลับเป็น JSON เท่านั้น: {"item": number, "price": number}
- ถ้าหาข้อมูลไม่เจอ หรือ ข้อมูลมาไม่ครบทั้ง item และ price ให้ตอบ: null"""
